  ...
  <div class="salary-export-container h-full flex flex-col">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden flex flex-col flex-1 min-h-0">
      ...
      >
        ...
        <el-table-column prop="workDays" label="Ngày công" width="110" align="center">
        ...
        <el-table-column label="Phụ cấp" width="140" align="right">
          <template #default="scope">
            <span class="text-blue-600 dark:text-blue-400 font-medium">{{ formatCurrency(scope.row.allowance) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="Thưởng" width="140" align="right">
          <template #default="scope">
        ...
        <el-table-column label="Trạng thái" width="130" align="center" fixed="right">
          <template #default="scope">
            <el-tag
              :type="scope.row.exported ? 'success' : 'info'"
              effect="dark"
              size="small"
              round
            >
              {{ scope.row.exported ? 'Đã xuất' : 'Chưa xuất' }}
            </el-tag>
          </template>
        </el-table-column>
      </el-table>
...
// Format currency for Excel (number only)
const formatCurrencyExcel = (value: number): string => {
...
const generateMockData = () => {
  ...
  for (let month = 1; month <= 6; month++) {
    employees.forEach((emp) => {
      ...
      const workDays = 22 + Math.floor(Math.random() * 5) - 2 // 20-26 days
      ...
      const penalty = unpaidLeave > 0 ? Math.round(baseSalary * unpaidLeave * 0.025) : 0
      const netSalary = baseSalary + overtimeSalary + allowance + bonus - socialInsurance - penalty
...
const generateExcel = (data: any[], fileName: string) => {
  ...
  const headerRows = [
    ...
    [],
    ['STT', 'Mã NV', 'Tên nhân viên', 'Ngày công', 'Lương cơ bản', 'Lương tăng ca', 'Phụ cấp', 'Thưởng', 'BHXH', 'Phạt', 'Thực nhận']
  ]

  const dataRows = data.map((row, index) => [
    index + 1, row.employeeCode, row.employeeName, row.workDays,
    row.baseSalary, row.overtimeSalary, row.allowance, row.bonus,
    row.socialInsurance, row.penalty, row.netSalary
  ])
  ...
  // Data rows
  const numberCols = [4, 5, 6, 7, 8, 9, 10]
